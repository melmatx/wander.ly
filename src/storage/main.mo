...
actor Assets {
  ...
  private let chunks = Map.new<Nat, Types.Chunk>();

  private let assets = Map.new<Text, Types.Asset>();
  ...
  public shared query ({ caller }) func http_request(
    ...
    if (request.method == "GET") {
      ...
      let asset : ?Types.Asset = Map.get(assets, thash, key);
      ...
      switch (asset) {
        ...
        case null {
          Debug.print("Asset not found: " # key);
        };
      };
    };
  ...
  public shared query ({ caller }) func http_request_streaming_callback(
    ...
    switch (Map.get(assets, thash, st.key)) {
  ...
  public shared ({ caller }) func commit_batch(
    ...
    var content_chunks = Buffer.Buffer<Blob>(chunk_ids.size());
